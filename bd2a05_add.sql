...
--   at:        2019-01-21 13:07:09 CET
--   site:      Oracle Database 11g
--   type:      Oracle Database 11g



CREATE user bd2a05 identified by account unlock 
;

CREATE user bd2a05_app identified by account unlock 
;

CREATE SEQUENCE ucz_seq START WITH 1 INCREMENT BY 1;
...
CREATE OR REPLACE TRIGGER BD2A05.tr_check_date 
...
declare t_date DATE;
        now_date DATE;
begin
    t_date := :new.okres_obowiazywania;
    now_date := TO_DATE('20-01-2019', 'dd/mm/yyyy');
    if t_date < now_date then
            DBMS_OUTPUT.PUT_LINE ('Wrong date!');
    end if;
end; 
/

CREATE OR REPLACE TRIGGER BD2A05.tr_check_price 
    AFTER INSERT ON Oferty 
    FOR EACH ROW 
    ENABLE 
declare t_cena NUMBER;
begin
    t_cena := 1000;
    --select :new.cena_wywolawcza into t_cena from oferty;
    if :new.cena_wywolawcza > t_cena then
        DBMS_OUTPUT.PUT_LINE ('Big price! The police are watching you!');
    end if;
end ; 
/

CREATE OR REPLACE TRIGGER BD2A05.tr_uczestnicy 
    BEFORE INSERT OR UPDATE OR DELETE ON Ilosci 
    FOR EACH ROW 
    ENABLE 
DECLARE
  v_user  VARCHAR2(15);
BEGIN
  ...
  IF INSERTING THEN
    DBMS_OUTPUT.PUT_LINE('one line inserted in uczestnicy by '||v_user);
  ELSIF DELETING THEN
    DBMS_OUTPUT.PUT_LINE('one line Deleted in uczestnicy by '||v_user);
  ELSIF UPDATING THEN
    DBMS_OUTPUT.PUT_LINE('one line Updated in uczestnicy by '||v_user);
  END IF;
END; 
/
